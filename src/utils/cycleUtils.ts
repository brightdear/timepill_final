...
function isInsideDateWindow(config: CycleConfig, dateKey: string): boolean {
  if (!('startDate' in config) && !('endDate' in config)) return true
...
export function isTodayDue(slot: SlotForCycle, checkDate = new Date()): boolean {
  ...
  if (!isInsideDateWindow(config, dateKey)) return false
